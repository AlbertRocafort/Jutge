...
void comprovaVertical(string paraula, const Matrix& sopa, int i, int j, int& puntuacio, int l) {
...
void comprovaHoritzontal(string paraula, const Matrix& sopa, int i, int j, int& puntuacio, int l) {
			...
			for (int j = 0; j < f; j++) {
				for (int k = 0; k < c; k++) {
					if (sopa[j][k].first == aux[0]) {
						if (j + tamany_paraula - 1 < f) comprovaVertical(aux, sopa, j, k, puntuacio, i);
						if (k + tamany_paraula - 1 < c) comprovaHoritzontal(aux, sopa, j, k, puntuacio, i);
					}
				}
			}
