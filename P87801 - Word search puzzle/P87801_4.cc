...
void comprovaVertical(string paraula, const Matrix& sopa, int i, int j, vector<int>& puntuacions, int l) {
	...
	while (k < llargada and not final and i+k < sopa.size()) {
...
void comprovaHoritzontal(string paraula, const Matrix& sopa, int i, int j, vector<int>& puntuacions, int l) {
	...
	while (k < llargada and not final and j+k < sopa[i].size()) {
			...
			for (int j = 0; j < f; j++) {
				for (int k = 0; k < c; k++) {
					if (sopa[j][k].first == aux[0]) {
						comprovaVertical(aux, sopa, j, k, puntuacions, i);
						comprovaHoritzontal(aux, sopa, j, k, puntuacions, i);
					}
				}
			}
